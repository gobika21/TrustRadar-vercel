...
export function TrustRadarLogo() {
  return (
    <div className="flex items-center gap-2.5">
      <span aria-hidden="true" className="relative inline-grid h-7.5 w-7.5 flex-none place-items-center rounded-full">
        <span
          className="radar-mark absolute inset-0 rounded-full"
          style={{
            background:
              "conic-gradient(from 220deg, var(--color-amber) 0deg, var(--color-amber-strong) 140deg, var(--color-line) 140deg 360deg)",
          }}
        />
        <span className="relative h-3 w-3 rounded-full bg-panel" />
      </span>
